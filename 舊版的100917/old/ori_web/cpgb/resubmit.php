...
{list($rname,$submit,$remail,$rcolor,$rsexual,$rip,$rR2,$rmsn,$rhome,$rhomename,$rR1,$rdate,$rhidden,$note)=explode("∥",$lines[$_GET['id']-1]);  
...
<body>
<center>
<table><tr><td><font size="2" color="<?=$gu_link;?>">│</font><a href="guest.php"><font color="<?=$gm_link;?>" size="2">回留言板</font></a><font size="2" color="<?=$gu_link;?>">│</font><a href="<?=$HTTP_SERVER_VARS['PHP_SELF'];?>?clear=yes&id=<?=$_GET['id'];?>"><font color="<?=$gm_link;?>" size="2">清除留言紀錄</font></a><font size="2" color="<?=$gu_link;?>">│</font></tr></table><br>
<table width=429  style="border: 1 <?=$gf_type;?> <?=$gm_line;?>" bgcolor="<?=$gm_skin;?>">
  <tr>
    <td width=100% style="border: 1 solid #C0C0C0"><font color=black size=2>留言內容：<br><?=$note;?></font>　</td>
  </tr>
</table>
<form action="re.php?id=<?=$rdate ;?>" method="post">
<table border="1" width="58%"   style="border: 1 <?=$gf_type;?> <?=$gm_line;?>" bgcolor="<?=$go_skin;?>">
  <tr>
    ...
    <td width="33%" style="border: 1 <?=$gf_type;?> <?=$gi_line;?>"><input type=hidden name=submit  value=$submit ><input type="text" name="cant" disabled size="19" value="<?=$submit ;?>"></td>
<?
}
?>
  ...
  <tr>
    ...
    <td width="33%" style="border: 1 <?=$gf_type;?> <?=$gi_line;?>"><select name="color" size="1"><option value="FFFFFF">FFFFFF
		...
		<option value="FF7C50" style="background-color:FF7C50 ;color: F7C50">FF7C50    
  ...
  <tr>
    ...
    <td width="33%" style="border: 1 <?=$gf_type;?> <?=$gi_line;?>"><select name="R2" disabled size="1">
    ...
    <td width="33%" style="border: 1 <?=$gf_type;?> <?=$gi_line;?>"><select name="R1" disabled  onChange="document.images['R1'].src=options[selectedIndex].value;" style="BACKGROUND-COLOR: #ffffff; BORDER-BOTTOM: 1px double; BORDER-LEFT: 1px double; BORDER-RIGHT: 1px double; BORDER-TOP: 1px double; COLOR: #000000">
<?
